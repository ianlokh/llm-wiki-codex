...
- Cite every substantive answer with a clickable relative Markdown link to a local path and heading, for example: `[wiki/concepts/attention.md#Key ideas](wiki/concepts/attention.md#key-ideas)` — the link target is the file path plus the heading slugified (lowercase, spaces to hyphens).
